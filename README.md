...
There are some functional interfaces that can be used to handle info, warn, debug, trace or error messages,
and exceptions.
...
operation, telling just if it succeeded or not (and eventually returns an error message back).

The StatusTracker class keeps track of whether an object changed its state or not. The provided implementation
...
So you need to:
- create a RootPathProvider (just use the default one, RootPathProviderImpl)
...
implementation is provided, that can go back and forth between all hints and remember if they are to be shown at
startup or not, tracking which one was the last shown.
